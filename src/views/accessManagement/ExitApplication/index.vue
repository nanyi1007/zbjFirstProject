<template>
  <div class="applyForAccess">
      ...
      <div class="top">
        <div class="content-box-check">
          ...
          <div class="ifBox ifBox2">
            ...
            >
              <el-option label="待执行" value="01" />
              <el-option label="执行中" value="02" />
            </el-select>
          </div>
        </div>
          ...
          <el-table :data="tableData">
            ...
            >
              <template slot-scope="scope">
                <span
                  :style="{ color: scope.row.status === '02' ? '#3D95E8' :
                    '' }"
                >
                  {{
                    scope.row.status === '01' ? '待执行' :
                    scope.row.status === '02' ? '执行中' :
                    ''
                  }}
                </span>
              </template>
            ...
            <el-table-column prop="time1" label="操作" align="center" width="250px">
              <template slot-scope="scope">
                <span v-if="scope.row.status === '01'" class="row">
                  <el-link type="primary" @click="exitApply(scope.row.consId)">退出申请</el-link>
                </span>
                <span v-if="scope.row.status !== '01'" class="row">
                  -
                </span>
    ...
    <el-dialog class="refuseReason" title="退出申请原因" :visible.sync="isVisible " width="25%" :modal="false" top="25vh">
      <el-form
        ref="form"
        :model="queryParam"
        label-width="100px"
